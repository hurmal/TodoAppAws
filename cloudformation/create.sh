...
aws cloudformation create-stack \
  --stack-name hurmal-basic-network \
...
aws cloudformation create-stack \
  --stack-name hurmal-basic-service \
  --template-body file://service.yml \
  --parameters \
      ParameterKey=NetworkStackName,ParameterValue=hurmal-basic-network \
      ParameterKey=ServiceName,ParameterValue=todo-app-v1 \
      ParameterKey=ImageUrl,ParameterValue=docker.io/hurmal/todo-app-v1:latest \
      ParameterKey=ContainerPort,ParameterValue=8080

aws cloudformation wait stack-create-complete --stack-name hurmal-basic-service

CLUSTER_NAME=$(aws cloudformation describe-stacks --stack-name hurmal-basic-network --output text --query 'Stacks[0].Outputs[?OutputKey==`ClusterName`].OutputValue | [0]')
